...
class RiskAnalytics:
    ...
    def calculate_beta(self, ticker: str, benchmark: str = 'SPY', period: str = '1y') -> Optional[float]:
        ...
        df_ticker = self._get_price_data(ticker, period)
        df_bench = self._get_price_data(benchmark, period)
        
        if df_ticker is None or df_bench is None:
            return None
        
        returns_ticker = df_ticker['Close'].pct_change().dropna()
        returns_bench = df_bench['Close'].pct_change().dropna()
        
        # Align dates
        aligned = pd.DataFrame({
            'ticker': returns_ticker,
        ...
        return round(float(cov / var_bench), 2)
    ...
    def get_all_risk_metrics(self, ticker: str, period: str = '1y') -> Dict:
        return {
            ...
            'beta': self.calculate_beta(ticker, 'SPY', period)
        }
